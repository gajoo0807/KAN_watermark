...
class KAN(nn.Module):
    ...
    def forward(self, x):
        ...
        self.acts.append(x)  # 將input加到激活列表中  acts shape: (batch, width[l])

        for l in range(self.depth):

            x_numerical, preacts, postacts_numerical, postspline = self.act_fun[l](x) 
            # x經過layer的output
            # 當前層計算後的數值激活、前激活、後激活、spline function輸出

            if self.symbolic_enabled == True: # 啟動符號計算
                x_symbolic, postacts_symbolic = self.symbolic_fun[l](x)
            else:
                x_symbolic = 0.
                postacts_symbolic = 0.

            x = x_numerical + x_symbolic # 數值激活+符號激活
            postacts = postacts_numerical + postacts_symbolic # 數值後激活+符號後激活(y軸)
        ...
        for l in range(depth):
            ...
            for i in range(self.width[l]): # input neurons
                for j in range(self.width[l + 1]): # output neurons
                    ...
                    plt.savefig(f'{folder}/sp_{l}_{i}_{j}.png', bbox_inches="tight", dpi=400) # l：從l傳到l+1, i：l層的第i個神經元, j：l+1層的第j個神經元
                    plt.close()
                ...
                if l < neuron_depth - 1:
                    ...
                    for j in range(n_next):
                        ...
                        if symbol_mask == 0. and numerical_mask == 0.:
                            color = "white"
                            alpha_mask = 0.
                        if mask == True: # 如果啟用了遮罩，則透明度會考慮當前和下一層的遮罩值。
                        ...
                        else: # 由顏色和透明度設定決定的透明度 alpha_mask。
    ...
    def train(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1., stop_grid_update_step=50, batch=-1,
        ...
        def reg(acts_scale):
            # 計算和返回激活尺度(acts_scale)、激活函數係數的 L1 正則化和激活的熵（entropy）的加權和。鼓勵模型學習更簡潔、更有效的數據表示，同時抑制過擬合。
